...
**Field Instrument No. 065 · Workshop Stores and Inventory · v1.0.0**
...
- **Test harness** in-app: 15 tests covering GSN assignment, ledger math, void rules, the muster state machine, and requisition thresholds, run against an isolated state without touching live data.

## Running it

Open `quartermaster-v1.0.0.html` in any modern browser. No build step, no dependencies, no network required. Data persists in the browser via local storage; if storage is unavailable the app runs in memory and warns you to export before closing.
...
- No QR or barcode scanning, no printable label sheet generator, and no floor-plan location map yet. All are on the deferred list.
